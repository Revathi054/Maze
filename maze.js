...
// Example movement function (modify based on your logic)
function movePlayer(key, player) {
    let validMove = false;

    switch (key) {
        case 'ArrowUp':
            if (player.y > 0 && !cells[player.x][player.y].walls.top) {
                player.y--;
                validMove = true;
            }
            break;
        case 'ArrowDown':
            if (player.y < rows - 1 && !cells[player.x][player.y].walls.bottom) {
                player.y++;
                validMove = true;
            }
            break;
        case 'ArrowLeft':
            if (player.x > 0 && !cells[player.x][player.y].walls.left) {
                player.x--;
                validMove = true;
            }
            break;
        case 'ArrowRight':
            if (player.x < cols - 1 && !cells[player.x][player.y].walls.right) {
                player.x++;
                validMove = true;
            }
            break;
    ...
    points += validMove ? 10 : -10;
    if (points < 0) points = 0; // Prevent negative score
...
// Button event listeners (for manual movement)
document.getElementById("btnUp").addEventListener("click", () => movePlayer('ArrowUp', player1));
document.getElementById("btnDown").addEventListener("click", () => movePlayer('ArrowDown', player1));
document.getElementById("btnLeft").addEventListener("click", () => movePlayer('ArrowLeft', player1));
document.getElementById("btnRight").addEventListener("click", () => movePlayer('ArrowRight', player1));
